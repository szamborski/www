...
class Prawa extends CI_Controller {
    ...
    public function index() {
        ...
        $data['posts'] = $this->posts->pokaz_userow();

        $this->load->view('admin/posts/users_view', $data);
    }

    public function zmien_prawa() {
        $prawo = $this->input->post('select', TRUE);
        $id = $this->input->post('id', TRUE);
        $this->posts->zmien_prawa($id, $prawo);
    }
    ...
    public function edit_acount() {
        ...
        $result = $this->posts->pokaz_usera($id);
        $data['info'] = $result;
        $this->load->view('admin/posts/acount_edit_view', $data);
        return;
    }
    ...
    public function save_profile() {
        ...
        if ($aff_rows > 0) {
            $this->output->set_output(json_encode(array("status" => 1, "message" => "Prifol został pomyślnie zaktualizowany")));
        } else {
            $this->output->set_output(json_encode(array("status" => 0, "message" => "Nie wprowadzono żandych zmian.")));
        }
    }
